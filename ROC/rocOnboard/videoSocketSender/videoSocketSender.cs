...
namespace videoSocketTools
{
    ...
    /*public class dualVideoSocketSender
    {
        ...
        private void transmitWideImage(Bitmap _leftImage, Bitmap _rightImage)
        {
            if (Transmitting && timeExpired)
            {
                    ...
                    try
                    {
                        wideImage = new Bitmap( (_leftImage.Width + _rightImage.Width) , _leftImage.Height);
                        using (Graphics g = Graphics.FromImage(wideImage))
                        {
                            g.DrawImage(_leftImage, 0, 0);
                            g.DrawImage(_rightImage, _leftImage.Width, 0);
                        }
                    }
                    catch
                    {
        ...
        private void rightCameraSource_NewFrame(object sender, NewFrameEventArgs eventArgs)
        {
            lock (camSync)
            {
                rightImage = eventArgs.Frame;
                RW.setRight(ImageToByte(rightImage));
                rightSourceReady = true;
                if (leftSourceReady)
                {
                    transmitWideImage(leftImage, eventArgs.Frame);
                }
            }
        }
        ...
        private void leftCameraSource_NewFrame(object sender, NewFrameEventArgs eventArgs)
        {
            lock (camSync)
            {
                leftImage = eventArgs.Frame;
                RW.setLeft(ImageToByte(leftImage));
                leftSourceReady = true;
                if (rightSourceReady)
                {
                    transmitWideImage(eventArgs.Frame, rightImage);
                }
            }
        }
    ...
    public class dualVideoSocketSender
    {
        ...
        private void transmitWideImage()
        {
            if (Transmitting && timeExpired)
            {
                ...
                Bitmap wideImage = DB.getCombined();
               // RW.setMerged(ImageToByte(wideImage));
